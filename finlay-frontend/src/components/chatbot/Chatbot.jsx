...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState([
    { id: 1, type: 'text', text: "Hi! I'm Qbit, your AI financial assistant. I've analyzed your portfolio—how can I help you today?", sender: 'ai' }
  ]);
  ...
  const handleSend = async (e) => {
    ...
    if (query.toUpperCase().includes("TSLA")) {
      // Step 1: Market Data
      setTimeout(() => {
        setMessages(prev => [...prev, {
          id: Date.now(),
      ...
      setTimeout(() => {
        setMessages(prev => [...prev, {
          id: Date.now() + 1,
          type: 'text',
          text: "Learning from past trades using Hindsight...",
          sender: 'ai',
          isLoading: true
        }]);
      }, 2500);

      // Step 3: Recommendation & Memory Stats
      setTimeout(() => {
        setMessages(prev => {
          const filtered = prev.filter(m => !m.isLoading); // remove loading message
          return [...filtered, {
            id: Date.now() + 2,
            type: 'recommendation',
            sender: 'ai',
            action: 'BUY',
            probability: 82,
            reason: 'Volume spike detected matching historical breakout patterns in our memory bank.',
            memoryStats: {
              tradesFound: 12,
              successRate: 68
            }
      ...
      setTimeout(() => {
        ...
        setMessages(prev => [...prev, {
          id: Date.now() + 1,
          type: 'text',
          text: "I am actively monitoring the markets. For a full demonstration of my capabilities, try asking me to 'Analyze TSLA'.",
          sender: 'ai'
        }]);
      }, 1000);
    }
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 w-full max-w-sm sm:max-w-md glass-panel rounded-2xl shadow-2xl flex flex-col overflow-hidden z-50 origin-bottom-right border border-white/10"
          >
            {/* Chat Header */}
            <div className="p-4 border-b border-white/10 bg-surface/90 flex justify-between items-center cursor-pointer" onClick={() => setIsMinimized(!isMinimized)}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-white flex items-center gap-2">
                    Qbit AI
                    <span className="text-[10px] uppercase font-bold bg-gradient-to-r from-primary to-accent px-1.5 py-0.5 rounded text-white">Pro</span>
                  </h3>
                  <div className="flex items-center gap-1.5 text-xs text-gray-400">
                    <div className="w-1.5 h-1.5 rounded-full bg-success animate-pulse"></div> Monitoring Markets
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={(e) => { e.stopPropagation(); setIsMinimized(!isMinimized); }} className="text-gray-400 hover:text-white transition">
                  {isMinimized ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                </button>
                <button onClick={(e) => { e.stopPropagation(); setIsOpen(false); }} className="text-gray-400 hover:text-white transition">
                  <X size={20} />
                </button>
              </div>
            </div>
              ...
              <div className="p-4 border-t border-white/10 bg-surface/90 relative z-10">
                <form onSubmit={handleSend} className="relative flex items-center">
                  <input
                    ...
                    placeholder="Ask Qbit about stocks, sentiment..."
                ...
                <div className="mt-3 flex gap-2 overflow-x-auto scrollbar-hide pb-1">
                  <button onClick={() => setInput("Analyze TSLA")} className="shrink-0 text-xs px-3 py-1.5 rounded-full border border-white/10 bg-surface text-gray-400 hover:text-white hover:border-white/30 transition-all whitespace-nowrap flex items-center gap-1.5">
                    <Activity size={12}/> Analyze TSLA
                  </button>
                  <button onClick={() => setInput("What's the market sentiment?")} className="shrink-0 text-xs px-3 py-1.5 rounded-full border border-white/10 bg-surface text-gray-400 hover:text-white hover:border-white/30 transition-all whitespace-nowrap">
                    Market Sentiment
                  </button>
                </div>
              </div>
